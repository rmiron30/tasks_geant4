...
# in acest program, se calculeaza total optical yield, entries, raportul nr de entries 
# la nr de particule din beam, nr de entries calculat analitic si raportul
# energiei depuse la energia totala a beam-ului
...
def getDataFromFiles(config):
    ...
    data["entriesRatio"] = entries/beam*100
    mu = att[data["material"]][data["energy"]]
    data["i_final"] = beam * (1 - np.exp(-mu * data["width"] / 10))
    data["analiticalRatio"] = data["i_final"]/beam*100
    data["energyRatio"] = eDep/(beam*float(config["energy"]))*100
...
output_file.write(
    "Mat\tE (MeV)\tWidth\tFWHM\tFWTM\teHalf\teTen\teDep\traport\tscintillations\tentries\tentriesComputed\tentriesRatio\tanaliticalRatio\tenergyRatio\n"
)
...
for dataPoint in data:
    ...
    output_file.write("\t")
    output_file.write(str(np.round(dataPoint["i_final"], 4)))
    output_file.write("\t")
    output_file.write(str(np.round(dataPoint["entriesRatio"], 4)))
    output_file.write("\t")
    output_file.write(str(np.round(dataPoint["analiticalRatio"], 4)))
    output_file.write("\t")
    output_file.write(str(np.round(dataPoint["energyRatio"], 4)))
    output_file.write("\n")
...
for dataPoint in data:
    if dataPoint["material"]=="CsI":
        if dataPoint["width"] == 0.5:
            xc05.append(dataPoint["energy"])
            yc05.append(dataPoint["entriesRatio"])
        elif dataPoint["width"] == 1:
            xc1.append(dataPoint["energy"])
            yc1.append(dataPoint["entriesRatio"])
        elif dataPoint["width"] == 2:
            xc2.append(dataPoint["energy"])
            yc2.append(dataPoint["entriesRatio"])
    elif dataPoint["material"]=="LYSO":
        if dataPoint["width"] == 0.5:
            xl05.append(dataPoint["energy"])
            yl05.append(dataPoint["entriesRatio"])
        elif dataPoint["width"] == 1:
            xl1.append(dataPoint["energy"])
            yl1.append(dataPoint["entriesRatio"])
        elif dataPoint["width"] == 2:
            xl2.append(dataPoint["energy"])
            yl2.append(dataPoint["entriesRatio"])
    elif dataPoint["material"]=="BGO":
        if dataPoint["width"] == 0.5:
            xb05.append(dataPoint["energy"])
            yb05.append(dataPoint["entriesRatio"])
        elif dataPoint["width"] == 1:
            xb1.append(dataPoint["energy"])
            yb1.append(dataPoint["entriesRatio"])
        elif dataPoint["width"] == 2:
            xb2.append(dataPoint["energy"])
            yb2.append(dataPoint["entriesRatio"])
...
plt.plot(xb2, yb2, color = colors_materials["BGO"], label = "BGO 2 mm", marker = markers_width["2.0"])
plt.xlabel('Energy (MeV)')
plt.ylabel("Entries/Incident photons (%)")
plt.title("Intrinsec efficiency versus energy")
plt.xlim([-0.5,10.5])
plt.yscale("log")
plt.legend()
plt.savefig("intrinsic_eff.pdf")
...
for dataPoint in data:
    if dataPoint["material"]=="CsI":
        if dataPoint["width"] == 0.5:
            xc05.append(dataPoint["energy"])
            yc05.append(dataPoint["energyRatio"])
        elif dataPoint["width"] == 1:
            xc1.append(dataPoint["energy"])
            yc1.append(dataPoint["energyRatio"])
        elif dataPoint["width"] == 2:
            xc2.append(dataPoint["energy"])
            yc2.append(dataPoint["energyRatio"])
    elif dataPoint["material"]=="LYSO":
        if dataPoint["width"] == 0.5:
            xl05.append(dataPoint["energy"])
            yl05.append(dataPoint["energyRatio"])
        elif dataPoint["width"] == 1:
            xl1.append(dataPoint["energy"])
            yl1.append(dataPoint["energyRatio"])
        elif dataPoint["width"] == 2:
            xl2.append(dataPoint["energy"])
            yl2.append(dataPoint["energyRatio"])
    elif dataPoint["material"]=="BGO":
        if dataPoint["width"] == 0.5:
            xb05.append(dataPoint["energy"])
            yb05.append(dataPoint["energyRatio"])
        elif dataPoint["width"] == 1:
            xb1.append(dataPoint["energy"])
            yb1.append(dataPoint["energyRatio"])
        elif dataPoint["width"] == 2:
            xb2.append(dataPoint["energy"])
            yb2.append(dataPoint["energyRatio"])
...
plt.plot(xb2, yb2, color = colors_materials["BGO"], label = "BGO 2 mm", marker = markers_width["2.0"])
plt.xlabel('Energy (MeV)')
plt.ylabel("Deposited energy / Total photon energy (%)")
# plt.title("Intrinsec efficiency versus energy")
plt.xlim([-0.5,10.5])
plt.yscale("log")
plt.legend()
plt.savefig("edep_versus_total.pdf")
