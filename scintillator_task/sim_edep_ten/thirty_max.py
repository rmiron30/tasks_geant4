...
def CalculateThirty(hist):
    tenthmax = hist.GetMaximum() / 30.0
...
def getDataFromFiles(config):
    ...
    data["fwtm"] = fwtm
    data["eDep"] = eDep
    data["eThirty"] = eThirty
    data["raport"] = eThirty/eDep*100
...
for root, dirs, files in os.walk(cwd):
     for dir in dirs:
            ...
            if histo:
                ...
                left, right, tenthmax, fwtm, eThirty = CalculateThirty(Xproj)
                eDep = 0
...
plt.xlabel('Energy (MeV)')
plt.ylabel("FW30M")
plt.title("FW30M as function of energy")
plt.xlim([-0.5,10.5])
plt.legend()
plt.savefig("FW30M_all.pdf")
